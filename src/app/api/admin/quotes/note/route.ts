...
export async function POST(request: Request) {
  ...
  const result = await updateQuoteRequestInternalNote(firestore, data.quoteId, data.internalNote);

  if (!result.ok) {
    return NextResponse.json({ error: result.error }, { status: result.status });
  }

  return NextResponse.json({ quoteId: result.quoteId, internalNote: result.internalNote });
}
