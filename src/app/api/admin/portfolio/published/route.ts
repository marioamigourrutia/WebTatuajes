...
export async function POST(request: Request) {
  ...
  const result = await updatePortfolioPublishedStatus(firestore, data.itemId, data.published);

  if (!result.ok) {
    return NextResponse.json({ error: result.error }, { status: result.status });
  }

  return NextResponse.json({ itemId: result.itemId, published: result.published });
}
