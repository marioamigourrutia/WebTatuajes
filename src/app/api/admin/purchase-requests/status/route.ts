...
export async function POST(request: Request) {
  ...
  const result = await updatePurchaseRequestStatus(firestore, data.purchaseRequestId, data.status);

  if (!result.ok) {
    return NextResponse.json({ error: result.error }, { status: result.status });
  }

  return NextResponse.json({ purchaseRequestId: result.purchaseRequestId, status: result.status });
}
