...
export function AdminSponsorsPanel({ enabled }: { enabled: boolean }) {
  ...
  async function loadSponsors() {
    ...
    try {
      ...
      const body = (await response.json()) as { sponsors?: Sponsor[]; error?: string };
      ...
      setError(
        caughtError instanceof Error ? caughtError.message : "No se pudo listar colaboradores.",
      );
      setSponsors([]);
    } finally {
      setLoading(false);
  ...
  async function createSponsor(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok || !body.id) {
        setError(Object.values(body.errors ?? {})[0] ?? "No se pudo crear el colaborador.");
        return;
      }

      form.reset();
      setNotice("Colaborador creado desde ruta server-side con rol admin validado.");
      await loadSponsors();
    } catch (caughtError) {
      setError(
        caughtError instanceof Error ? caughtError.message : "No se pudo crear el colaborador.",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function saveSponsor(sponsor: Sponsor, active = sponsor.active) {
    ...
    try {
      ...
      if (!response.ok || !body.id) {
        setError(Object.values(body.errors ?? {})[0] ?? "No se pudo actualizar el colaborador.");
        return;
      }

      setSponsors((current) =>
        current.map((item) => (item.id === sponsor.id ? { ...item, active } : item)),
      );
      setNotice("Colaborador actualizado.");
    } catch {
      setError("No se pudo conectar con la ruta server-side de colaboradores.");
    } finally {
  ...
  return (
    <section className="space-y-4 rounded-3xl border border-stone-800 bg-stone-900/50 p-5">
      <div>
        <h3 className="text-xl font-bold text-stone-50">Colaboradores y auspiciadores</h3>
        <p className="mt-1 text-sm text-stone-400">
          Gestiona marcas, estudios aliados o proveedores visibles en la web pública.
        </p>
      </div>
      ...
      >
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <label className="grid gap-1 text-sm font-semibold text-stone-200">
            ...
            <input
              ...
              maxLength={80}
              name="category"
              placeholder="Proveedor, marca aliada, estudio invitado"
              required
            />
          </label>
          <label className="grid gap-1 text-sm font-semibold text-stone-200">
        ...
        <div className="flex flex-wrap items-center gap-4">
          <label className="grid gap-1 text-sm font-semibold text-stone-200">
            Orden
            <input
              className="w-28 rounded-xl border border-stone-700 bg-stone-900 px-3 py-2 text-stone-100"
              defaultValue={0}
              min={0}
              max={999}
              name="sortOrder"
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between gap-3">
          <h4 className="font-bold text-stone-100">Colaboradores registrados</h4>
          <button
              ...
              >
                <div className="flex gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="font-semibold text-stone-100">{sponsor.name}</p>
                    <p className="text-sm text-stone-400">{sponsor.category}</p>
                    <p className="text-xs text-stone-500">{formatDate(sponsor.createdAt)}</p>
                  </div>
                </div>
                <p className="mt-3 line-clamp-3 text-sm text-stone-300">{sponsor.description}</p>
                <div className="mt-4 flex flex-wrap items-center gap-3">
                  <label className="flex items-center gap-2 text-sm text-stone-200">
                    <input
                      checked={sponsor.active}
                      disabled={updatingSponsorId === sponsor.id}
                      onChange={(event) => saveSponsor(sponsor, event.target.checked)}
                      type="checkbox"
                    />
                    Visible
                  </label>
                  <button
                    className="rounded-full border border-red-300/50 px-3 py-1 text-sm text-red-100"
                    disabled={updatingSponsorId === sponsor.id}
