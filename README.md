...
- Firebase es la dirección backend oficial para Auth admin, Firestore, Admin SDK server-only y Security Rules. La cotización pública ya no requiere verificación de email de Firebase.
- Cualquier requisito previo de Supabase/PostgreSQL/RLS debe traducirse a equivalentes Firebase. Supabase no es dependencia ni proveedor requerido.
...
- El flujo de cotización persiste una solicitud mínima server-side para que admin vea calendario/datos de cliente y luego abre WhatsApp con el código creado. Las referencias/fotos se envían por WhatsApp, no por el formulario público.
...
7. Abre `http://localhost:3000/quote`, completa la solicitud de cotización y envíala. El resultado esperado es un mensaje de éxito con el código local y la apertura de WhatsApp para enviar referencias/fotos por chat.
...
9. En **Portafolio administrable**, crea un ítem con título, estilo, zona del cuerpo, descripción corta, etiquetas separadas por coma y, opcionalmente, una URL pública de imagen. Marca **Publicar en `/portfolio`** si quieres verlo públicamente.
...
El formulario público no abre escrituras cliente en Firestore Rules: la creación pasa por `/api/quotes`, conserva validación/bot protection/rate limit y usa Admin SDK server-side. El servidor guarda la cotización mínima y, si hay fecha tentativa, la reserva visible para admin. Las referencias visuales no se guardan en el formulario público; se envían por WhatsApp con el código de cotización.

El portafolio administrable tampoco abre escrituras públicas/cliente para crear contenido: `/api/admin/portfolio` y `/api/admin/portfolio/published` vuelven a validar ID token y rol admin server-side antes de escribir en `portfolio_items`. En modo sin proveedor externo, las imágenes admin usan URLs públicas/manuales.

Las previews de imágenes del panel admin no exponen secretos del proveedor. Para Cloudinary se mantiene la URL segura existente; para datos legacy de Storage local, el cliente admin obtiene blobs mediante `/api/admin/quotes/images?imageId=...`, enviando el ID token en `Authorization`.
...
Para mantener compatibilidad con Firebase Spark y presupuesto cero, el flujo recomendado es URL-only/manual:

1. Mantén `IMAGE_UPLOAD_PROVIDER=disabled`.
2. Para cotizaciones públicas, las referencias/fotos se mandan por WhatsApp con el código creado; no se suben al formulario.
3. Si ya existen credenciales Cloudinary, puedes configurar `IMAGE_UPLOAD_PROVIDER=cloudinary`; no es requerido para cotizaciones públicas.
4. Si se habilita un proveedor compatible, el servidor valida tipo declarado, magic bytes y tamaño, genera un id aleatorio no identificable y guarda solo metadata y provider id en Firestore.

Antes de subir a un proveedor externo compatible, el servidor decodifica las imágenes con Sharp, aplica auto-orientación EXIF, redimensiona sin agrandar dentro de límites orientados (horizontal 1920×1080, vertical 1080×1920, cuadradas/otras 1920×1920), elimina metadata al no usar `keepMetadata()` y recomprime a WebP calidad 82. GIF queda rechazado por ahora con error explícito porque el procesamiento seguro de animaciones está fuera de alcance.

Cloudinary queda solo como compatibilidad si ya existen credenciales previas; la guía nueva no requiere proveedor de uploads.
...
### Cotizaciones públicas y WhatsApp

El formulario público de `/quote` no exige inicio de sesión ni verificación por email. `/api/quotes` valida los campos, aplica bot protection/rate limit, crea la cotización en Firestore y devuelve los datos necesarios para abrir WhatsApp con el código generado. Las fotos o referencias se envían por WhatsApp, no se suben desde el formulario público.

Firebase Auth sigue usándose para administración. No habilites email-link como requisito del flujo público de cotización salvo que se rediseñe explícitamente el producto.
