...
- Configurar Supabase solo si se habilita almacenamiento privado de imágenes: URL, service role y bucket de cotizaciones.
...
- Revisar CSP en `next.config.ts` y en variables `CSP_*_EXTRA` contra dominios reales de Firebase, Supabase, WhatsApp, Instagram y cualquier CDN de imágenes.
...
- Confirmar que `/api/quotes` exige token bearer de email verificado.
...
- `npm run test`
- `npm run build`
- Smoke manual: cotización con email verificado, rechazo sin verificar, admin cambia estado/abono/reserva, comunidad, compra y opinión.

## Checklist de aceptación

- Cotización pública acepta solo clientes con email verificado y rechaza anónimos/no verificados con mensaje claro.
...
- Build de Vercel usa solo credenciales server-side para Firebase Admin, Supabase e Instagram.

## Validación post-deploy

- Confirmar headers de seguridad en una ruta pública.
- Enviar una cotización real de prueba con email verificado y revisar trazabilidad en Firestore.
...
- Configurar credenciales reales en Vercel/Firebase/Supabase/Meta. No hay credenciales reales en el repositorio.
